// runner-ui.js – upgraded for full test button output display
document.addEventListener('DOMContentLoaded', () => {
  ...
  function displayDetailedResults(title, items) {
    let output = `=== ${title} ===\n`;
    if (items.length === 0) {
      ...
      items.forEach(item => {
        if (typeof item === 'string') {
          ...
          output += `- ${item.name} (${item.private ? 'Private' : 'Public'})\n`;
        } else if (item.collection) {
          output += `• ${item.collection}\n  Docs: ${item.documents.join(', ') || "none"}\n`;
        }
      });
    }
    appendOutput(output);
  ...
  document.getElementById('testFirebaseBtn').addEventListener('click', () => {
    fetch(`${backendBaseURL}/testFirebase`)
      .then(res => res.json())
      .then(data => {
        displayDetailedResults("Firebase Collections", data.data || []);
      });
  });

  document.getElementById('testGitHubBtn').addEventListener('click', () => {
    fetch(`${backendBaseURL}/testGitHub`)
      .then(res => res.json())
      .then(data => {
        displayDetailedResults("GitHub Repositories", data.data || []);
      });
  });

  document.getElementById('testVercelBtn').addEventListener('click', () => {
    fetch(`${backendBaseURL}/testVercel`)
      .then(res => res.json())
      .then(data => {
        displayDetailedResults("Vercel Projects", data.data || []);
      });
  });
  ...
  showTemplates();
  checkStatusBar();
  setInterval(checkStatusBar, 30000); // periodic refresh every 30s
