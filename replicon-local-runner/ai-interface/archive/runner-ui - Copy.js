...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function checkStatusBar() {
    ...
    Promise.all([
      fetch('/testFirebase').then(r => r.json()).then(d => d.success ? "Firebase ✅" : "Firebase ❌"),
      fetch('/testGitHub').then(r => r.json()).then(d => d.success ? "GitHub ✅" : "GitHub ❌"),
      fetch('/testVercel').then(r => r.json()).then(d => d.success ? "Vercel ✅" : "Vercel ❌")
    ]).then(results => {
  ...
  function submitToRunner() {
    ...
    fetch('/run', {
      ...
      body: JSON.stringify({ actionType: actionType.value, payload })
    })
    .then(res => res.json())
    .then(data => {
  ...
  document.getElementById('testFirebaseBtn').addEventListener('click', () => fetch('/testFirebase'));
  document.getElementById('testGitHubBtn').addEventListener('click', () => fetch('/testGitHub'));
  document.getElementById('testVercelBtn').addEventListener('click', () => fetch('/testVercel'));
